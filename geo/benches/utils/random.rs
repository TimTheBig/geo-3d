...
use geo_3d::algorithm::{BoundingRect, ConcaveHull, ConvexHull, MapCoords, Rotate};
use geo_3d::geometry::*;
...
pub fn uniform_point<R: Rng>(rng: &mut R, bounds: Rect<f64>) -> Coord<f64> {
    let coords: [f64; 3] = rng.sample(StandardUniform);
...
pub fn steppy_polygon<R: Rng>(mut rng: R, steps: usize) -> Polygon<f64> {
    ...
    ring.push((0.0, 0.0, 0.0).into());
    (0..steps).for_each(|_| {
        let x: f64 = rng.sample::<f64, _>(StandardUniform);
        let z: f64 = rng.sample::<f64, _>(StandardUniform);
        y += y_step;
        ring.push((x, y, z).into());
    });
    ring.push((x_shift, y, 0.0).into());
    (0..steps).for_each(|_| {
        let x: f64 = rng.sample::<f64, _>(StandardUniform);
        let z: f64 = rng.sample::<f64, _>(StandardUniform);
        y -= y_step;
        // y += normal.sample(&mut rng);
        ring.push((x_shift + x, y, z).into());
    });
    ring.push((0.0, y_step, 0.0).into());
    (0..steps).for_each(|_| {
        let x: f64 = rng.sample::<f64, _>(StandardUniform);
        let z: f64 = rng.sample::<f64, _>(StandardUniform);
        // y += normal.sample(&mut rng);
        ring.push((x, y, z - z_shift).into());
    });

    normalize_polygon(Polygon::new(LineString(ring), vec![]))
}

/// Normalizes polygon to fit and fill `[-1, 1, -1] X [-1, 1, -1]` square.
///
