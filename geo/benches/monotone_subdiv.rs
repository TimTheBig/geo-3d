...
use geo_3d::coordinate_position::CoordPos;
use geo_3d::monotone::monotone_subdivision;
use geo_3d::{CoordinatePosition, MapCoords, Polygon};
...
fn criterion_benchmark_pt_in_poly(c: &mut Criterion) {
    let pt_samples = Samples::from_fn(512, || {
        uniform_point(&mut rng(), Rect::new((-1., -1., -1.), (1., 1., 1.)))
    });
