...
#[test]
fn test_one_point() {
    let mut v = vec![coord! { x: 0., y: 0., z: 0. }];
    let correct = vec![coord! { x: 0., y: 0., z: 0. }, coord! { x: 0., y: 0., z: 0. }];
    let res = trivial_hull(&mut v, false);
    assert_eq!(res.0, correct);
}
...
#[test]
fn test_two_points() {
    let mut v = vec![coord! { x: 0., y: 0., z: 0. }, coord! { x: 1., y: 1., z: 1. }];
    let correct = vec![
        coord! { x: 0., y: 0., z: 0. },
        coord! { x: 1., y: 1., z: 1. },
        coord! { x: 0., y: 0., z: 0. },
...
fn test_two_points_duplicated() {
    let mut v = vec![coord! { x: 0., y: 0., z: 0. }, coord! { x: 0., y: 0., z: 0. }];
    let correct = vec![coord! { x: 0., y: 0., z: 0. }, coord! { x: 0., y: 0., z: 0. }];
    let res = trivial_hull(&mut v, false);
    assert_eq!(res.0, correct);
}
...
fn convex_hull_multilinestring_test() {
    let v1 = line_string![(x: 0.0, y: 0.0, z: 0.0), (x: 1.0, y: 10.0, z: 1.0)];
    let v2 = line_string![(x: 1.0, y: 10.0, z: 1.0), (x: 2.0, y: 0.0, z: 2.0), (x: 3.0, y: 1.0, z: 3.0)];
    let mls = MultiLineString::new(vec![v1, v2]);
    let correct = vec![
        Coord::from((2.0, 0.0, 2.0)),
        Coord::from((3.0, 1.0, 3.0)),
        Coord::from((1.0, 10.0, 1.0)),
...
fn convex_hull_multipolygon_test() {
    let p1 = polygon![(x: 0.0, y: 0.0, z: 0.0), (x: 1.0, y: 10.0, z: 1.0), (x: 2.0, y: 0.0, z: 2.0), (x: 0.0, y: 0.0, z: 0.0)];
    let p2 = polygon![(x: 3.0, y: 0.0, z: 3.0), (x: 4.0, y: 10.0, z: 4.0), (x: 5.0, y: 0.0, z: 5.0), (x: 3.0, y: 0.0, z: 3.0)];
    let mp = MultiPolygon::new(vec![p1, p2]);
    let correct = vec![
        Coord::from((5.0, 0.0, 5.0)),
        Coord::from((4.0, 10.0, 4.0)),
        Coord::from((1.0, 10.0, 1.0)),
        Coord::from((0.0, 0.0, 0.0)),
        Coord::from((5.0, 0.0, 5.0)),
    ];
