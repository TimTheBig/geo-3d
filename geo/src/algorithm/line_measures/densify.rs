...
pub trait Densify<F: CoordNum> {
    type Output;
    fn densify<Euclidean>(&self, max_segment_length: F) -> Self::Output
    where
        Euclidean: Distance<F, Point<F>, Point<F>> + InterpolatePoint<F>;
}

pub(crate) fn densify_between<F, Euclidean>(
    line_start: Point<F>,
    line_end: Point<F>,
    ...
    Euclidean: Distance<F, Point<F>, Point<F>> + InterpolatePoint<F>,
{
    ...
    let num_segments = (Euclidean::distance(line_start, line_end) / max_segment_length)
        .ceil()
        .to_u64()
...
impl<F: CoordNum + FromPrimitive> Densify<F> for LineString<F> {
    ...
    fn densify<Euclidean>(&self, max_segment_length: F) -> LineString<F>
    where
        Euclidean: Distance<F, Point<F>, Point<F>> + InterpolatePoint<F>,
    {
...
impl<F: CoordNum + FromPrimitive> Densify<F> for MultiLineString<F> {
        ...
        Euclidean: Distance<F, Point<F>, Point<F>> + InterpolatePoint<F>,
    {
        MultiLineString::new(
            self.iter()
                .map(|line_string| line_string.densify::<Euclidean>(max_segment_length))
                .collect(),
        )
    }
}
...
impl<F: CoordNum + FromPrimitive> Densify<F> for Polygon<F> {
    ...
    {
        Polygon::new(
            self.exterior().densify::<Euclidean>(max_segment_length),
            self.interiors()
                .iter()
                .map(|interior| interior.densify::<Euclidean>(max_segment_length))
                .collect(),
        )
    }
}
...
impl<F: CoordNum + FromPrimitive> Densify<F> for MultiPolygon<F> {
        ...
        Euclidean: Distance<F, Point<F>, Point<F>> + InterpolatePoint<F>,
    {
        MultiPolygon::new(
            self.iter()
                .map(|polygon| polygon.densify::<Euclidean>(max_segment_length))
                .collect(),
        )
    }
}
...
impl<F: CoordNum + FromPrimitive> Densify<F> for Rect<F> {
    ...
    where
        Euclidean: Distance<F, Point<F>, Point<F>> + InterpolatePoint<F>,
    {
        self.to_polygon().densify::<Euclidean>(max_segment_length)
    }
}
...
mod tests {
    use super::*;
    use crate::{coord, polygon, wkt, Euclidean};
    ...
    fn densify_polygon() {
        ...
        let densified_polygon = polygon.densify::<Euclidean>(500_000.0); // 500 km max segment length
