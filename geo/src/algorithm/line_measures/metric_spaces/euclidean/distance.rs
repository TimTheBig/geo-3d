...
impl<F: CoordNum> Distance<F, Point<F>, Line<F>> for Euclidean {
    fn distance(point: Point<F>, line: Line<F>) -> F {
        ::geo_types::private_utils::point_line_euclidean_distance(point, line)
    }
}
...
mod test {
    ...
    fn distance1_test() {
        assert_relative_eq!(
            Euclidean::distance(&Point::new(0., 0., 0.0), &Point::new(1., 0., 0.0)),
            1.
        );
    }
    ...
    fn distance2_test() {
        let dist = Euclidean::distance(
            &Point::new(-72.1235, 42.3521, 0.0),
            &Point::new(72.1260, 70.612, 0.0)
        );
    ...
    fn test_large_polygon_distance() {
        let ls = geo_test_fixtures::norway_main::<f64>(); // assume these fixtures now yield 3D coords (or wrap with z:0.0)
